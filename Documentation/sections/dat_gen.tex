...
Podemos ver la estructura básica de la red en la figura \ref{fig:neuralNetwork}.\newline

\begin{figure}[h]
    \tikzset{%
      every neuron/.style={
        circle, draw, fill=blue!20, minimum size=1cm
      },
      neuron missing/.style={
        draw=none, scale=4,text height=0.333cm, fill=none, execute at begin node=\color{black}$\vdots$
      },
    }
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        \node[functions] (centre) {\tikz \draw[scale=0.11,domain=-2.141:2.141,smooth,variable=\t] %
        plot (\t,{sin(\t r)});};
        \node[below of=center,font=\scriptsize,text width=3em,align=center] {activation function};
    ...
    \caption{Estructura de la Neurona}
    \label{fig:completeNeuron}
\end{figure}

Cada nodo representa una neurona, en la figura \ref{fig:completeNeuron} se representa el funcionamiento de cada una de ellas y se puede computar de la siguiente forma.

\[y=\sigma(\sum_{i=0}^{N}x_{i}*w_{i})\]
\newline

Donde sigma representa la función de activación, $x_i$ los inputs de la neurona y $w_i$ los pesos correspondientes.

Su estructura es muy similar a la del perceptron con la diferencia de que en lugar de una funcion de signo, su salida pasa por una función de activación (tanh, sigmoid, ReLU...) que modifica la salida en lugar de simplemente tomar una salida de +1 o -1.

Por ejemplo, la función ReLU(x) tomará 0 como salida para cualquier x con valor negativo mientras que tomará valor x para cualquier valor positivo de x.
...
\begin{algorithm}[H]
    ...
    Feed forward pass\\
    \For{$layer \gets 0$ to $layers$}
    {
        \For{$neuron \gets 0$ to $layerSize$}
        {
	...
	\For{$neuron \gets 0$ to $outputLayerSize$}
	{
	    node.activationPrimeError(outputs, expectedOutputs);
	}
	
	Gradient for hidden layers
	\For{$layer \gets layers[layersSize - 1]$ to $0$} 
	{
	    \For{$neuron \gets 0$ to $layerSize$} 
	    {   
	        $nextLayer = layer + 1;$
	        ...
	        \For{$nextLayerNeuron \gets 0$ to $nextLayer$} 
	        {
...
\begin{algorithm}
    \For{$epoch \gets 0$ to $totalEpochs$}
    {
        \For{$layer \gets 0$ to $layers$}
        {
            \For{$neuron \gets 0$ to $layerSize$} 
            {
                 \If{$dropout[neuron] < RandomDouble(0.0, 1.0)$}
                 {
                    $neuron.disable()$
                 }
            }    
...
\begin{algorithm}
     ...
     \For{$k \gets 0$ to $folds$} 
     {
        $currentSampleIndex = k * perFoldSamples$
        
        \For{$input \gets 0$ to $inputsSize$} 
        {
            \eIf{$input.belongsToK(currentSampleIndex,input,k)$}
            {
